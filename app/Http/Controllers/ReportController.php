...
class ReportController extends Controller
{
        ...
        try {
            switch ($tipo) {
                ...
                case 'epi13':
                    ...
                    $fechaInicio = Carbon::now()->setISODate($anio, $numSem)->startOfWeek();
    ...
    public function exportEpi12(Request $request)
    {
        ...
        $fechaInicio = Carbon::now()->setISODate((int) $anio, (int) $numeroSemana)->startOfWeek();
        $fechaFin = $fechaInicio->copy()->endOfWeek();
        ...
        foreach ($consultations as $consultation) {
            if ($consultation->sisDiagnoses && $consultation->sisDiagnoses->isNotEmpty()) {
                foreach ($consultation->sisDiagnoses as $consultaDiag) {
                    ...
                    if (! $claveEnf || ! isset($datosEpi12[$claveEnf])) {
                        continue;
                    }

                    // Calcular grupo de edad (13 grupos del oficial)
                    $edad = $patient->birth_date ? Carbon::parse($patient->birth_date)->age : null;
        ...
        foreach ($consultations as $consultation) {
            if ($consultation->sisDiagnoses && $consultation->sisDiagnoses->isNotEmpty()) {
                foreach ($consultation->sisDiagnoses as $consultaDiag) {
                    ...
                    $edad = $patient->birth_date ? Carbon::parse($patient->birth_date)->age : '';

                    $casos[] = [
                        'fecha' => Carbon::parse($consultation->consultation_date)->format('d/m/Y'),
                        'nombre' => $patient->full_name ?? '',
                        'ci' => ($patient->nationality ?? 'V').'-'.($patient->id_number ?? ''),
                        'genero' => strtoupper($patient->gender ?? ''),
                        'fecha_nac' => $patient->birth_date ? Carbon::parse($patient->birth_date)->format('d/m/Y') : '',
                        'edad' => $edad,
    ...
    private function clasificarEnfermedadEpi12($diagnosis): ?string
    {
        ...
        // Cólera (A00)
        if (strpos($codigo, 'A00') === 0) {
            return 'A00';
        }

        // Diarreas (A08-A09)
        if (strpos($codigo, 'A08') === 0 || strpos($codigo, 'A09') === 0 || strpos($nombre, 'diarrea') !== false) {
            return 'A08-A09';
        }

        // Amibiasis (A06)
        if (strpos($codigo, 'A06') === 0 || strpos($nombre, 'amibi') !== false || strpos($nombre, 'amebi') !== false) {
            return 'A06';
        }
        ...
        if (strpos($codigo, 'B15') === 0) {
            return 'B15';
        }

        // Tuberculosis (A15-A19)
        if (preg_match('/^A1[5-9]/', $codigo) || strpos($nombre, 'tuberculosis') !== false || strpos($nombre, 'tb ') !== false) {
            return 'A15-A19';
        }

        // Influenza (J10-J11)
        if (preg_match('/^J1[01]/', $codigo) || strpos($nombre, 'influenza') !== false || strpos($nombre, 'gripe') !== false) {
            return 'J10-J11';
        }
        ...
        if (strpos($codigo, 'Z21') === 0) {
            return 'Z21';
        }

        // Enfermedad VIH/SIDA (B20-B24)
        if (preg_match('/^B2[0-4]/', $codigo) || strpos($nombre, 'hiv') !== false || strpos($nombre, 'sida') !== false) {
            return 'B20-B24';
        }

        // Tosferina (A37)
        if (strpos($codigo, 'A37') === 0 || strpos($nombre, 'tosferina') !== false || strpos($nombre, 'tos ferina') !== false) {
            return 'A37';
        }

        // Parotiditis (B26)
        if (strpos($codigo, 'B26') === 0 || strpos($nombre, 'parotiditis') !== false || strpos($nombre, 'paperas') !== false) {
            return 'B26';
        }

        // Tétanos Neonatal (A33)
        if (strpos($codigo, 'A33') === 0) {
            return 'A33';
        }

        // Tétanos Obstétrico (A34)
        if (strpos($codigo, 'A34') === 0) {
            return 'A34';
        }

        // Tétanos otros (A35)
        if (strpos($codigo, 'A35') === 0) {
            return 'A35';
        }

        // Difteria (A36)
        if (strpos($codigo, 'A36') === 0 || strpos($nombre, 'difteria') !== false) {
            return 'A36';
        }

        // Sarampión Sospecha (B05)
        if (strpos($codigo, 'B05') === 0 || strpos($nombre, 'sarampión') !== false || strpos($nombre, 'sarampion') !== false) {
            return 'B05';
        }
        ...
        if (strpos($nombre, 'dengue') !== false) {
            if (strpos($nombre, 'grave') !== false || strpos($codigo, 'A91') === 0) {
                return 'A91';
            }
            if (strpos($nombre, 'alarma') !== false) {
                return 'DENGUE_CON';
            }
        ...
        // Chikungunya
        if (strpos($nombre, 'chikungunya') !== false) {
            return 'CHIKUNGUNYA';
        }

        // Zika (U08)
        if (strpos($codigo, 'U08') === 0 || strpos($nombre, 'zika') !== false) {
            return 'U08';
        }

        // Encefalitis Equina Venezolana (A92.2)
        if (strpos($codigo, 'A92.2') === 0 || strpos($nombre, 'encefalitis equina') !== false) {
            return 'A92.2';
        }
        ...
        if (strpos($codigo, 'B55.0') === 0 || (strpos($nombre, 'leishmaniasis') !== false && strpos($nombre, 'visceral') !== false)) {
            return 'B55.0';
        }

        // Leishmaniasis Cutánea (B55.1)
        if (strpos($codigo, 'B55.1') === 0 || (strpos($nombre, 'leishmaniasis') !== false && strpos($nombre, 'cutánea') !== false)) {
            return 'B55.1';
        }

        // Leishmaniasis Mucocutánea (B55.2)
        if (strpos($codigo, 'B55.2') === 0 || (strpos($nombre, 'leishmaniasis') !== false && strpos($nombre, 'mucocut') !== false)) {
            return 'B55.2';
        }

        // Leishmaniasis no Específica (B55.9)
        if (strpos($codigo, 'B55') === 0 || (strpos($nombre, 'leishmaniasis') !== false && strpos($nombre, 'específ') === false && strpos($nombre, 'visceral') === false && strpos($nombre, 'cutánea') === false && strpos($nombre, 'mucocut') === false)) {
            return 'B55.9';
        }

        // Chagas Aguda (B57.0-B57.1)
        if (preg_match('/^B57\.[01]/', $codigo) || (strpos($nombre, 'chagas') !== false && strpos($nombre, 'aguda') !== false)) {
            return 'B57.0-B57.1';
        }

        // Chagas Crónica (B57.2-B57.5)
        if (preg_match('/^B57\.[2-5]/', $codigo) || (strpos($nombre, 'chagas') !== false && strpos($nombre, 'crónica') !== false)) {
            return 'B57.2-B57.5';
        }

        // Rabia Humana (A82)
        if (strpos($codigo, 'A82') === 0 || strpos($nombre, 'rabia humana') !== false) {
            return 'A82';
        }

        // Fiebre Hemorrágica Venezolana (A96.8)
        if (strpos($codigo, 'A96.8') === 0 || strpos($nombre, 'fiebre hemorrágica venezolana') !== false) {
            return 'A96.8';
        }

        // Leptospirosis (A27)
        if (strpos($codigo, 'A27') === 0 || strpos($nombre, 'leptospirosis') !== false) {
            return 'A27';
        }

        // Meningitis Viral (A87)
        if (strpos($codigo, 'A87') === 0 || (strpos($nombre, 'meningitis') !== false && strpos($nombre, 'viral') !== false)) {
            return 'A87';
        }

        // Meningitis Bacteriana (G00)
        if (strpos($codigo, 'G00') === 0 || (strpos($nombre, 'meningitis') !== false && strpos($nombre, 'bacteriana') !== false)) {
            return 'G00';
        }

        // Enfermedad Meningococcica (A39.0)
        if (strpos($codigo, 'A39.0') === 0) {
            return 'A39.0';
        }

        // Enfermedad Meningococcica (A39.9)
        if (strpos($codigo, 'A39.9') === 0 || strpos($codigo, 'A39') === 0) {
            return 'A39.9';
        }

        // Varicela (B01)
        if (strpos($codigo, 'B01') === 0 || strpos($nombre, 'varicela') !== false) {
            return 'B01';
        }

        // Hepatitis Aguda Tipo B (B16)
        if (strpos($codigo, 'B16') === 0) {
            return 'B16';
        }

        // Hepatitis Aguda Tipo C (B17.1, B18.2)
        if (strpos($codigo, 'B17.1') === 0 || strpos($codigo, 'B18.2') === 0) {
            return 'B17.1_B18.2';
        }

        // Hepatitis Otras Agudas (B17)
        if (strpos($codigo, 'B17') === 0 || (strpos($nombre, 'hepatitis') !== false && strpos($nombre, 'aguda') !== false && strpos($codigo, 'B15') !== 0 && strpos($codigo, 'B16') !== 0 && strpos($codigo, 'B17.1') !== 0 && strpos($codigo, 'B18.2') !== 0)) {
            return 'B17';
        }

        // Hepatitis No Específicas (B19)
        if (strpos($codigo, 'B19') === 0) {
            return 'B19';
        }

        // Parálisis Flácida < 15 años (G82.0)
        if (strpos($codigo, 'G82') === 0 || strpos($nombre, 'parálisis flácida') !== false) {
            return 'G82.0';
        }

        // Neumonías (J12-J18)
        if (preg_match('/^J1[2-8]/', $codigo) || strpos($nombre, 'neumonía') !== false || strpos($nombre, 'neumonia') !== false) {
            return 'J12-J18';
        }

        // Intoxicación por Plaguicidas (T60)
        if (strpos($codigo, 'T60') === 0 || strpos($nombre, 'plaguicida') !== false || strpos($nombre, 'pesticida') !== false || strpos($nombre, 'intoxicación') !== false) {
            return 'T60';
        }

        // Mordedura Sospechosa de Rabia (A82)
        if (strpos($nombre, 'mordedura') !== false && strpos($nombre, 'rabia') !== false) {
            return 'A82_RABIA';
        }

        // Fiebre (R50)
        if (strpos($codigo, 'R50') === 0 || strpos($nombre, 'fiebre') !== false) {
            return 'R50';
        }

        // Efectos Adversos de Medicamentos (Y40-Y57)
        if (preg_match('/^Y4[0-9]/', $codigo) || preg_match('/^Y5[0-7]/', $codigo) || strpos($nombre, 'efecto adverso') !== false || strpos($nombre, 'efectos adversos') !== false) {
            return 'Y40-Y57';
        }

        // Efectos Adversos de Vacunas (Y58-Y59)
        if (preg_match('/^Y5[89]/', $codigo) || strpos($nombre, 'vacuna') !== false) {
            return 'Y58-Y59';
        }
    ...
    private function normalizarSexo($sexo): string
    {
        if (strtoupper($sexo) === 'M') {
            return 'H';
        } // Hombre (EPI-12 convention)
        if (strtoupper($sexo) === 'F') {
            return 'M';
        } // Mujer (EPI-12 convention)
    ...
    private function clasificarEnfermedadEpi15(?string $codigo, ?int $edad = null): ?int
    {
        ...
        if ($edad !== null) {
            foreach ($candidatos as $c) {
                $edadOk = true;
                if (isset($c['emin']) && $edad < $c['emin']) {
                    $edadOk = false;
                }
                if (isset($c['emax']) && $edad > $c['emax']) {
                    $edadOk = false;
                }
                if ($edadOk) {
                    return $c['n'];
            ...
            if ($consultation->sisDiagnoses && $consultation->sisDiagnoses->isNotEmpty()) {
                foreach ($consultation->sisDiagnoses as $consultaDiag) {
                    ...
                    if ($numEnf !== null && isset($consolidado[$numEnf])) {
                        if ($tipo === 'P') {
                            $consolidado[$numEnf]['p']++;
                        } elseif ($tipo === 'S') {
                            $consolidado[$numEnf]['s']++;
                        } else {
                            $consolidado[$numEnf]['x']++;
                        }
                    }
                }
            }
    ...
    public function historical(Request $request): Response
    {
        ...
        $sectors = Patient::whereNotNull('addr_sector')->distinct()->pluck('addr_sector')->sort()->values();
    ...
    public function epiMatrixData(Request $request): JsonResponse
    {
        ...
        $fechaInicio = Carbon::now()->setISODate($year, $week)->startOfWeek();
        $fechaFin = $fechaInicio->copy()->endOfWeek();
        ...
        foreach ($consultations as $consultation) {
            ...
            if ($consultation->sisDiagnoses && $consultation->sisDiagnoses->isNotEmpty()) {
                foreach ($consultation->sisDiagnoses as $consultaDiag) {
                    ...
                    $tieneDiag = true;
                    $claveEnf = $this->clasificarEnfermedadEpi12($diagnosis);
                    if (! $claveEnf || ! isset($rows[$claveEnf])) {
                        continue;
                    }

                    $edad = $consultation->patient?->birth_date
                        ? Carbon::parse($consultation->patient->birth_date)->age
                        : null;
                    $grupoEdad = $this->obtenerGrupoEdadEpi12($edad);
        ...
        foreach ($consultations as $c) {
            ...
            if ($c->patient && ! $c->patient->birth_date) {
                $sinFechaNac[$c->patient->id] = $c->patient;
            }
            if ($c->patient && ! $c->patient->gender) {
                $sinSexo[$c->patient->id] = $c->patient;
            }
            if ($c->patient && ! $c->patient->addr_sector) {
                $sinSector[$c->patient->id] = $c->patient;
            }
