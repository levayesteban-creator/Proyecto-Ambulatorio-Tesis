...
class ExportController extends Controller
{
    ...
    private function getPatientFilters(Request $request): array
    {
        $filters = [];
        if ($request->gender) {
            $filters['Género'] = $request->gender === 'M' ? 'Masculino' : 'Femenino';
        }
        if ($request->status) {
            $filters['Estado'] = $request->status === 'closed' ? 'Cerrado' : 'Abierto';
        }
        if ($request->age_min || $request->age_max) {
            $filters['Edad'] = ($request->age_min ?? '0').' - '.($request->age_max ?? '∞');
        }
        if ($request->date_from || $request->date_to) {
            $filters['Fecha Registro'] = ($request->date_from ?? 'Inicio').' a '.($request->date_to ?? 'Fin');
        }
        ...
        $callback = function () use ($consultations) {
            ...
            foreach ($consultations as $c) {
                fputcsv($handle, [
                    ...
                    match ($c->consultation_type) {
                        'P' => 'Primera Vez', 'S' => 'Sucesiva', 'X' => 'Asociada', default => ''
                    },
                    $c->doctor?->name ?? '',
    ...
    private function getConsultationFilters(Request $request): array
    {
        ...
        if ($request->consultation_type) {
            $filters['Tipo'] = match ($request->consultation_type) {
                'P' => 'Primera Vez', 'S' => 'Sucesiva', 'X' => 'Asociada', default => ''
            };
        }
        if ($request->filled('is_healthy')) {
            $filters['Estado'] = $request->boolean('is_healthy') ? 'Sano' : 'Enfermo';
        }
        if ($request->date_from || $request->date_to) {
            $filters['Fecha'] = ($request->date_from ?? 'Inicio').' a '.($request->date_to ?? 'Fin');
        }
    ...
    private function getAuditFilters(Request $request): array
    {
        ...
        if ($request->action) {
            $filters['Acción'] = match ($request->action) {
                'created' => 'Creación', 'updated' => 'Actualización', 'deleted' => 'Eliminación', default => $request->action
            };
        }
        if ($request->date_from || $request->date_to) {
            $filters['Fecha'] = ($request->date_from ?? 'Inicio').' a '.($request->date_to ?? 'Fin');
